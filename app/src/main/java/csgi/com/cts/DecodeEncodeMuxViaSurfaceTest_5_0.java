 ...
 */
public class DecodeEncodeMuxViaSurfaceTest_5_0 {
    private static final String TAG = DecodeEncodeMuxViaSurfaceTest_5_0.class.getSimpleName();
    ...
    public DecodeEncodeMuxViaSurfaceTest_5_0(String input, String output, long start, long end) {
        inputFilePath = input;
        ...
        public void run() {
            ...
            for (int i = 0; i < extractor.getTrackCount(); i++) {
                ...
                if (mime.startsWith("video/")) {
                    ...
                    outputSurface = new OutputSurface(format.getInteger(MediaFormat.KEY_WIDTH), format.getInteger(MediaFormat.KEY_HEIGHT));
                    decoder.configure(format, outputSurface.getSurface(), null, 0);
                    outputSurface.makeCurrent();

                    if (endPosition <= startPosition + 100) {   // Too small
                        endPosition = format.getLong(MediaFormat.KEY_DURATION);
                    }

                    MediaFormat mediaFormat = MediaFormat.createVideoFormat("video/avc",
                            480, //format.getInteger(MediaFormat.KEY_WIDTH),
                            320); //format.getInteger(MediaFormat.KEY_HEIGHT));
...
//                        }
                        ...
                        if (doRender) {
                            ...
                            inputSurface.setPresentationTime((info.presentationTimeUs - startPosition) * 1000);
        ...
        private boolean offerMuxer() {
            ...
            while (true) {
                int encoderStatus = -1;
                try {
                    encoderStatus = encoder.dequeueOutputBuffer(bufferInfo, 100000);
